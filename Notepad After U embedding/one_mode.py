...
class OneMode(object):
    ...
    def v_load_data(self,graph_file,weighted,directed):
        #---------------------------------------STEP1: load data-----------------------------------------------------
        print('\nSTEP1: start loading data......')
        t1 = time.time()
        #load graph structure info------
    ...
    def u_embedding(self,method,ABRW_alpha,ABRW_topk,number_walks,walk_length,window_size,workers,dim, save_emb,emb_file):
        ...
        model = None
        if  method == 'abrw': 
            model = abrw.ABRW(graph=self.g, dim=dim, alpha=ABRW_alpha, topk=ABRW_topk, number_walks=number_walks,
                            walk_length=walk_length, window=window_size, workers=workers)
    ...
    def v_embedding(self,method,abrw_alpha,ABRW_topk,number_walks,walk_lenght,window_size,workers):
        #-----------------------------------STEP3: upstream embedding task-------------------------------------------------
        print('\nSTEP3: start learning embeddings......')
        print(f'the graph: {graph_file}; \nthe model used: {method}; \
            \nthe # of edges used during embedding (edges maybe removed if lp task): {self.g_v.get_num_edges()}; \
            \nthe # of nodes: {self.g_v.get_num_nodes()}; \nthe # of isolated nodes: {self.g_v.get_num_isolates()}; \nis directed graph: {self.g_v.get_isdirected()}')
        t1 = time.time()
        model = None
        if  method == 'abrw': 
            model = abrw.ABRW(graph=self.g_v, dim=args.dim, alpha=args.ABRW_alpha, topk=args.ABRW_topk, number_walks=args.number_walks,
