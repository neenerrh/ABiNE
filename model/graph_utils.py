...
class GraphUtils(object):
    ...
    def homogeneous_graph_random_walks_for_large_bipartite_graph(self, percentage, maxT, minT):
        ...
        #R = np.memmap('filename.dat', dtype=np.float32, mode='w+',shape=(A.shape[0], AT.shape[1]))
        #R[1,1]=5
        #print(R)
        ...
        #R = csr_matrix([ ], shape=(10,10))
        #R = csr_matrix([A_sparse.shape[0], AT_sparse.shape[1]])
        
      
     
        #for i in range(A.shape[0]):
            #for j in range(AT.shape[1]):
                #R[i,j] = np.dot(A[i,:], AT[:,j] )
                #print(R[i,j])
